...
public class DynamicFunctionStrategy extends Strategy implements Parallelizable {
    ...
    private String invokeString(String id) {
        ...
        String invoke =
                "com.surftheedge.tesseract.DynamicFunctionAllocationManager.invoke(" + InternalRepresentation.convert(id)
                        + ", $args)";
    ...
    private String anonymousFunction() {
        FunctionSignature function = ((DynamicFunctionExpression) getExpression()).getFunction();
        return "public static " + function.getReturnType() + " " + generateAnonymousName() + "("
                + function.getFunctionParameters() + ")" + ((DynamicFunctionExpression) getExpression()).getBody();
    }
    ...
    private String callableWrapperFunction(String id) {
        ...
        String baseTemplate =
                "public static " + function.getStub() + "{" + "Object[] $args = "
                        + ((function.getArguments().size() > 0) ? "{" + args + "}" : "null") + ";" +
